...
let obstacles=[{
    image:obstacle1
    ,width:100,height:104,cropw:563,croph:567,frame:0,y:410
},{
    image:obstacle2
    ,width:130,height:90,cropw:781,croph:547,frame:0,y:430
}]
...
function animate() {
 ...
 // ctx.drawImage(layer4,x,0)
  //ctx.drawImage(layer4,x2,0)
  //ctx.drawImage(layer2,x,0)
  //ctx.drawImage(layer2,x2,0)
...
function generateCoins() {
    //console.log(platforms[platforms.length-1].position.y)
    if (st==false) {
    ...
    setTimeout(generateCoins, presetTime1);}
}
...
function jump() {
    ...
    if(start && jump_avail==true && player.position.y>=canvas.offsetTop-60){
        jump1()
       }
    }
...
function generateBlocks() {
    ...
    setTimeout(generateBlocks, presetTime1);}
}
...
function isPastBlock(player, block){
    ...
    return(
       /* player.position.x + (player.width / 2) > block.x + (block.size / 4) && 
        player.position.x + (player.width / 2) < block.x + (block.size / 4) * 3
    */
...
function play() {
 ...
 jump_avail =true
  gameSpeed=3
   presetTime = Math.random()*(7000-3500)+3500;
 presetTime1 =  Math.random()*(7000-3500)+3500;
 arrayBlocks = [];
 enemySpeed1 = 3;
 enemySpeed = 3;
 score=0
